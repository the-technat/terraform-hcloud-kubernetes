...
| <a name="input_cluster_subnet_cidr"></a> [cluster\_subnet\_cidr](#input\_cluster\_subnet\_cidr) | A valid CIDR within the cluster\_vpc\_cidr for the cluster subnet | `string` | `"10.123.0.0/24"` | no |
...
| <a name="input_ip_mode"></a> [ip\_mode](#input\_ip\_mode) | Should all components talk to each other over IPv4 or IPv6? (IPv6 saves costs) | `string` | `"ipv6"` | no |
...
| <a name="input_master_nodes"></a> [master\_nodes](#input\_master\_nodes) | List of master nodes to provision in the cluster, each master node has a set of values you can uniqly configure | <pre>list(object({<br>    name        = string<br>    server_type = string<br>    image       = string<br>    labels      = map(string)<br>    location    = string<br>    volumes = list(object({<br>      name    = string<br>      size_gb = number<br>    }))<br>  }))</pre> | <pre>[<br>  {<br>    "image": "debian-11",<br>    "labels": {},<br>    "location": "hel1",<br>    "name": "master-0",<br>    "server_type": "cpx11",<br>    "volumes": []<br>  }<br>]</pre> | no |
| <a name="input_network_region"></a> [network\_region](#input\_network\_region) | In which network region should your cluster network be? (eu-central, us-east) | `string` | n/a | yes |
...
| <a name="input_ssh_keys"></a> [ssh\_keys](#input\_ssh\_keys) | List of (public) ssh keys to configure on the ssh\_user | `list(string)` | n/a | yes |
| <a name="input_ssh_port"></a> [ssh\_port](#input\_ssh\_port) | The SSH Port configured and rechable on each machine | `number` | `22` | no |
...
| <a name="input_ssh_user"></a> [ssh\_user](#input\_ssh\_user) | Username of a admin user created on every machine (used by ansible) | `string` | `"ansible"` | no |
| <a name="input_worker_nodes"></a> [worker\_nodes](#input\_worker\_nodes) | List of worker nodes to provision in the cluster, each master node has a set of values you can uniqly configure | <pre>list(object({<br>    name        = string<br>    server_type = string<br>    image       = string<br>    labels      = map(string)<br>    location    = string<br>    volumes = list(object({<br>      name    = string<br>      size_gb = number<br>    }))<br>  }))</pre> | <pre>[<br>  {<br>    "image": "debian-11",<br>    "labels": {},<br>    "location": "nbg1",<br>    "name": "worker-0",<br>    "server_type": "cpx31",<br>    "volumes": []<br>  }<br>]</pre> | no |
